...
void VectorBatch::set_col(int j,const std::vector<float> &v ) {
  ...
  #pragma omp parallel for shared(v, j, vals)
...
void VectorBatch::add_vector( const std::vector<float> &v ) {
  ...
  #pragma omp parallel for shared(v, vals)
...
std::vector<float> VectorBatch::get_col(int j) const {
  ...
  #pragma omp parallel for shared(col, j)
  for (int i=0; i<c; i++)
...
void VectorBatch::set_row( int j, const std::vector<float> &v ) {
  const int c = item_size();
  assert( v.size()==c );
  #pragma omp parallel for shared(vals, j, v)
  for (int i = 0; i < c; i++) {
...
std::vector<float> VectorBatch::get_row(int j) const {
  ...
  #pragma omp parallel for shared(row, j)
  for (int i = 0; i < n; i++)
...
std::vector<float> VectorBatch::extract_vector(int j) const {
  ...
  #pragma omp parallel for shared(v, j)
  for (int i = 0; i < m; i++)
...
void VectorBatch::set_vector( const Vector &v, int j) {
  const int c = item_size();
  assert( v.size()==c );
  #pragma omp parallel for shared(vals, v, j)
  for (int i=0; i<c; i++)
...
void VectorBatch::addh(const Vector &y) { // Add y to every row
  ...
  assert( r==y.size() );
  #pragma omp parallel for shared(vals, y)
  for (int j=0; j<c; j++ ) {
    for (int i=0; i<r; i++) {
...
Vector VectorBatch::meanh() const { // Returns a vector of row-wise means
  const int r = item_size(), c = batch_size();
  Vector mean(r, 0);
  #pragma omp parallel for shared(mean, vals)
  for (int i=0; i<r; i++) {
    float avg = 0.f;
    for ( int j=0; j<c; j++ ) {
...
VectorBatch VectorBatch::operator-(const VectorBatch &m2) {
  ...
  #pragma omp parallel for shared(out, m2)
    for (int i = 0; i < r * c; i++) {
...
void VectorBatch::hadamard(const VectorBatch& m1,const VectorBatch& m2) {
  ...
  #pragma omp parallel for shared(vals, m1vals, m2vals)
  for (int i=0; i<r*c; i++) {
...
void VectorBatch::scaleby( float f) {
  #pragma omp parallel for shared(f, vals)
  for (int i = 0; i < nelements(); i++) {
    vals[i] /= f;
  }
}
...
VectorBatch VectorBatch::operator-() {
    ...
    #pragma omp parallel for shared(result, vals)
    for (int i = 0; i < nelements(); i++) {
...
VectorBatch operator/(const VectorBatch &m, const float &c) {
    VectorBatch o = m;
    #pragma omp parallel for shared(m, c, o)
...
VectorBatch operator*(const float &c, const VectorBatch &m) {
    VectorBatch o = m;
    #pragma omp parallel for shared(o, c, m)
    for (int i = 0; i < m.nelements(); i++) {
        o.vals[i] = o.vals[i] * c;
    }
    return o;
