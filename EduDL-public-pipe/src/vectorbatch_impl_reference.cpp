...
// y = x x
void VectorBatch::v2mp(const Matrix &m, VectorBatch &y, int start, int end) const {
  const int
  ...
  int xi{0},yi{0},mi{0};
  for (int i = 0; i < yr; i++) { // Matrix multiplication subroutine, for each number in the vector
    for (int j = start; j < end; j++) { // for each thing in the batch
      float sum = 0.0;
      for (int k = 0; k < mc; k++) { // #matrix columns
	sum += ELEMENTr( mmat,i,k,mr,mc ) * ELEMENTc( xvals,k,j,xr,xc ) ; //both are in row-major order
  ...
  int index_diff = end - start;

  if (yc - end < index_diff) {
