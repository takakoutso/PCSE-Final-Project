...
Net::Net( const Dataset &data, int nStages ) {
  this->inR = data.data_size(); //data.items.at(0).data.size();
  this->inC = 1;
  samples = 0;  
  this->numStages = nStages;
}
...
void Net::train( const Dataset &train_data,const Dataset &test_data,
    ...
    for (int i_epoch = 0; i_epoch < epochs; i_epoch++) {
      ...
      for (int j = 0; j < batches.size(); j++) {
        ...
        int microsec_duration = std::chrono::duration_cast<std::chrono::microseconds>(duration).count();
        total_time += microsec_duration;
    ...
    cout << " Time taken on feedForward: " << total_time << "us" << endl;
...
void Net::allocate_batch_specific_temporaries(int batchsize) {
  ...
  for (int i = 0; i < layers.size(); i++)
    layers[i].allocate_batch_specific_temporaries(batchsize);
}
