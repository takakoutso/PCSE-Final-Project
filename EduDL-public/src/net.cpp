...
void Net::feedForward(const VectorBatch &input) {
  ...
  for (unsigned i = 0; i < layers.size(); i++) {
    // cout << "thread " << omp_get_thread_num() << " handling " << i << endl;
    if (i == 0) {
      #pragma omp task priority(layers.size()) shared(completions)
      {
      ...
      #pragma omp task priority(layers.size()-i) shared(completions)
      {
        for (int stage = 0; stage < numStages; stage++) {
          while (completions[(i-1)*numStages+stage] == false)   { asm("pause"); /* cout << "Here1 " <<  i << endl; */ }
          this->layers.at(i).forward(this->layers.at(i - 1).activated_batch, stage, numStages);
          completions[i*numStages+stage] = true;
        }
      }
    }
  }
...
void Net::backPropagate(const VectorBatch &input, const VectorBatch &gTruth) {
  ...
  if (layers.size()==1) {
    ...
    for (unsigned i = layers.size() - 2; i > 0; i--) {
      ...
      layers.at(i).backward( layers.at(i+1).delta, layers.at(i+1).weights, layers.at(i-1).activated_batch);
    }
...
void Net::train( const Dataset &train_data,const Dataset &test_data,
  ...
  microsec_duration += std::chrono::duration_cast<std::chrono::microseconds>(duration).count();
	...
	backPropagate(batch.inputs(),batch.labels());
      ...
      cout << " Accuracy on test set: " << acc << endl;
    }

    cout << "Time taken for feedforward: " << microsec_duration << "us" << endl;
...
float Net::calculateLoss(const Dataset &testSplit) {
  ...
  // assert( result.notnan() ); CHANGED
	...
	// assert( not std::isnan(this_result) ); CHANGED
