...
void Layer::forward(const VectorBatch &prevVals, unsigned stage, unsigned numStages) {
#ifdef DEBUG
  cout << "Forward layer " << layer_number
       << ": " << input_size() << "->" << output_size() << endl;
#endif

    // while (layer_idx > 0 && pipe[(layer_idx-1)*2] == false) {

    // } // spin while we wait for a half-batch to come through

    unsigned batch_idx_start = (prevVals.batch_size() * (stage  )) / numStages;
    unsigned batch_idx_end   = (prevVals.batch_size() * (stage+1)) / numStages;

    // cout << stage << " " << numStages << " " << batch_idx_start << " " << batch_idx_end << " " << activated_batch.batch_size() << endl;

    // cout << "start: " << batch_idx_start << "  end: " << batch_idx_end << endl;
